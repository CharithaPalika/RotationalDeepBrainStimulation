...
def STN_GPe_loop(exp_yaml_path):
    ...
    for i in tqdm(range(time)):
        ...
        if i % arguments.get('t_sector_activation') == 0:
            if arguments.get('sector_wise'):
                dbs_spread_ = np.zeros((stn_gpe_units,stn_gpe_units))
                if sector == 0:
                    dbs_spread_[0: int(stn_gpe_units//2), 0: int(stn_gpe_units//2)] = dbs_spread[0: int(stn_gpe_units//2), 0: int(stn_gpe_units//2)]
                elif sector == 1:
                    dbs_spread_[int(stn_gpe_units//2): int(stn_gpe_units), 0: int(stn_gpe_units//2)] = dbs_spread[int(stn_gpe_units//2): int(stn_gpe_units), 0: int(stn_gpe_units//2)]
                elif sector == 2:
                    dbs_spread_[int(stn_gpe_units//2): int(stn_gpe_units), int(stn_gpe_units//2): int(stn_gpe_units)] = dbs_spread[int(stn_gpe_units//2): int(stn_gpe_units), int(stn_gpe_units//2): int(stn_gpe_units)]           
                elif sector == 3:
                    dbs_spread_[0: int(stn_gpe_units//2), int(stn_gpe_units//2): int(stn_gpe_units)] = dbs_spread[0: int(stn_gpe_units//2), int(stn_gpe_units//2): int(stn_gpe_units)]
                
                # if i < 400:
                #     plt.imshow(dbs_spread_)
                #     plt.show()
